	...
	private static int [] decodeDXT3(int width, int height, int offset, byte [] buffer, Order order) {
		...
		for(int i=0; i<h; i++) {
			for(int j=0; j<w; j++) {
				...
				for(int k=0; k<4; k++) {
                    int a = (buffer[index++] & 0xff) | ((buffer[index++] & 0xff) << 8);
					// 4bit alpha to 8bit alpha
					alphaTable[4*k+0] = 17 * (((a) >>> 0) & 0xF) * 17;
					alphaTable[4*k+1] = 17 * (((a) >>> 4) & 0xF) * 17;
					alphaTable[4*k+2] = 17 * (((a) >>> 8) & 0xF) * 17;
					alphaTable[4*k+3] = 17 * (((a) >>> 12) & 0xF) * 17;
				}
				int c0 = (buffer[index] & 0xFF) | (buffer[index+1] & 0xFF) << 8; index += 2;
				int c1 = (buffer[index] & 0xFF) | (buffer[index+1] & 0xFF) << 8; index += 2;
				for(int k=0; k<4; k++) {
